...
def convert_mask(labels: np.array, masks: np.array):
    """Convert Mask R-CNN mask into the standart mask of shape (W, H)

    Args:
        labels (np.array): Labels
        masks (np.array): Binary masks of shape (Num labels, 1, W, H)

    Returns:
        np.array: Mask of shape (W, H)
    """
    new_mask = np.zeros((512, 512))
    
    for ind, label in enumerate(labels):
        new_mask[masks[ind, 0, :, :]] = label
